...
# ==========================================================
#                     OBJECT COUNTER CLASS (CORRECTED)
# ==========================================================
...
class ObjectCounter:
    def __init__(self, source, model="best_float32.tflite",
        ...
        if isinstance(source, str) and source.startswith("rtsp://"):
            ...
            if not self.cap.isOpened():
                raise Exception(f"❌ Failed to open video: {source}")
            self.is_rtsp = False
    ...
    def run(self):
        ...
        while True:
            # ✅ CORRECTED: Handle frame reading for both RTSP and File
            if self.is_rtsp:
                frame = self.cap.read()  # VideoStream returns frame directly
                ...
                ret, frame = self.cap.read()  # VideoCapture returns (ret, frame)
                if not ret:
                    print("📹 End of video file")
                    break
                    
                if frame is None:
                    print("⚠️ Invalid frame from video file")
                    break
        ...
        if self.is_rtsp:
            self.cap.stop()  # Stop VideoStream
        else:
