...
class ObjectCounter(BaseSolution):
    ...
    Attributes:
        ...
        track_side (dict[int, str]): Dictionary to track which side of the line each object is on.
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.track_side = {}  # Track which side of the line each object is on

        self.show_in = self.CFG["show_in"]
        self.show_out = self.CFG["show_out"]
        self.margin = self.line_width * 2  # Scales the background rectangle size to display counts properly

    def get_line_side(self, point: tuple[float, float]) -> str:
        ...
        Returns:
            str: 'left'/'right' for vertical lines, 'top'/'bottom' for horizontal lines.
        """
        if len(self.region) == 2:
            # Determine if line is vertical or horizontal
            if abs(self.region[0][0] - self.region[1][0]) < abs(self.region[0][1] - self.region[1][1]):
                # Vertical line - compare x-coordinates
                line_x = (self.region[0][0] + self.region[1][0]) / 2
                return 'right' if point[0] > line_x else 'left'
            else:
                # Horizontal line - compare y-coordinates
                line_y = (self.region[0][1] + self.region[1][1]) / 2
                return 'bottom' if point[1] > line_y else 'top'
        return None
    ...
    def count_objects(
        ...
        if len(self.region) == 2:  # Linear region (defined as a line segment)
            current_side = self.get_line_side(current_centroid)
            
            # If this is the first time seeing this track, just record its side
            ...
            if current_side != prev_side:
                ...
                if abs(self.region[0][0] - self.region[1][0]) < abs(self.region[0][1] - self.region[1][1]):
                    # Vertical line: left to right is IN, right to left is OUT
                    if current_side == 'right':  # Moving from left to right
                        self.in_count += 1
                    ...
                    # Horizontal line: top to bottom is IN, bottom to top is OUT
                    if current_side == 'bottom':  # Moving from top to bottom
                        self.in_count += 1
                        self.classwise_count[self.names[cls]]["IN"] += 1
                ...
                # Mark as counted and update side
                self.counted_ids.append(track_id)
                self.track_side[track_id] = current_side
            else:
                # Update the side even if not crossing (for accurate tracking)
                self.track_side[track_id] = current_side

        elif len(self.region) > 2:  # Polygonal region
            if self.r_s.contains(self.Point(current_centroid)):
                # Only count if we have previous position
