...
class ObjectCounter:
    def __init__(self, source, model="best_float32.tflite",
        ...
        # ✅ NEW: Track which side object first appeared on
        ...
        # ✅ MISSED LOGIC - Now based on origin side + crossing
        self.missed_in = set()
        self.missed_out = set()
    ...
    def check_lost_ids(self):
        ...
        Logic:
        - If object crossed line but wasn't counted:
          * If it came from IN side → Missed IN
          * If it came from OUT side → Missed OUT
        - If object never crossed → Missed Cross
        """
        ...
        for tid in lost:
            ...
            if tid in self.crossed_ids and tid not in self.counted:
                ...
                if origin == "IN":
                    ...
                    print(f"⚠️ MISSED IN - ID:{tid} (came from IN side, crossed but not counted)")
                elif origin == "OUT":
                    self.missed_out.add(tid)
                    print(f"⚠️ MISSED OUT - ID:{tid} (came from OUT side, crossed but not counted)")
        ...
        while True:
            ...
            if results[0].boxes.id is not None and self.line_p1:
                ...
                for tid, box in zip(ids, boxes):
                    ...
                    # ✅ NEW: Record origin side when first seen
                    if tid not in self.hist:
                        s_init = self.side(cx, cy, *self.line_p1, *self.line_p2)
                        if s_init < 0:
                            self.origin_side[tid] = "IN"  # Negative side = IN side
                        else:
                            self.origin_side[tid] = "OUT"  # Positive side = OUT side
                        ...
                        if s1 * s2 < 0:  # Crossed the line
                            ...
                            if tid not in self.counted:
                                ...
                                if s2 > 0:  # Going IN (positive side)
                                    ...
                                    print(f"✅ IN - ID:{tid} Color:{color_name} (origin: {self.origin_side.get(tid)})")
                                    ...
                                    print(f"✅ OUT - ID:{tid} Color:{color_name} (origin: {self.origin_side.get(tid)})")
