...
class ObjectCounter:
    def __init__(self, source, model="best_float32.tflite",
        ...
        print(f"📁 Using polygon file: {self.json_file}")
        ...
        if isinstance(source, str) and source.startswith("rtsp://"):
            ...
            time.sleep(2.0)
            self.is_rtsp = True
            print(f"📡 RTSP Stream connected: {source}")
        else:
        ...
        polygon_loaded = self.load_polygon()
        
        # Print polygon status
        if self.polygon:
            print(f"✅ Polygon is ready with {len(self.polygon_points)} points")
            print(f"   Coordinates: {self.polygon_points}")
        else:
            print(f"⚠️ No polygon loaded - click 4 points to create one")

        # Only setup mouse callback if polygon wasn't loaded
        if not polygon_loaded:
            cv2.namedWindow("ObjectCounter")
            cv2.setMouseCallback("ObjectCounter", self.mouse_event)
        else:
            cv2.namedWindow("ObjectCounter")
            # Still set callback in case user wants to redraw
            cv2.setMouseCallback("ObjectCounter", self.mouse_event)
    ...
    def load_polygon(self):
        """Load polygon coordinates from JSON file - Returns True if successful"""
        if os.path.exists(self.json_file):
            try:
                with open(self.json_file) as f:
                    ...
                    # Convert to tuples and create polygon
            ...
            print(f"ℹ️ No saved polygon found at {self.json_file}")
            print(f"   Click 4 points to create one.")
            return False
    ...
    def run(self):
        print("\n" + "="*60)
        print("CONTROLS:")
        print("  • Press 'O' to Reset counts and show summary")
        print("  • Press 'C' to Clear polygon and redraw")
        print("  • Press 'ESC' to Exit")
        print("="*60 + "\n")
        
        if not self.polygon:
            print("⚠️ WAITING FOR POLYGON - Click 4 points on the video window")
        else:
            print("✅ POLYGON LOADED - Tracking started automatically")

        while True:
            if self.is_rtsp:
                frame = self.cap.read()
                if frame is None:
                    print("⚠️ No frame from RTSP stream")
                    time.sleep(0.1)
                    continue
            else:
            ...
            # Draw temporary points (while creating polygon)
            ...
            # Draw complete polygon (ALWAYS if it exists)
            if self.polygon_points and len(self.polygon_points) == 4:
                ...
                # Fill with semi-transparent cyan overlay
                overlay = frame.copy()
            ...
            # Run detection and tracking ONLY if polygon exists
            if self.polygon:
            ...
            # Show polygon status
            if not self.polygon:
                cv2.putText(frame, "Click 4 points to create polygon", 
                           (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            else:
                cv2.putText(frame, "Polygon: ACTIVE", 
...
if __name__ == "__main__":
    # Example usage with RTSP stream
    counter = ObjectCounter(
        source="rtsp://your_camera_ip:554/stream",  # Your RTSP stream URL
        model="best_float32.tflite",
        classes_to_count=[0],
        show=True,
        json_file="polygon_coords.json"  # Will auto-load from this file
    )
    counter.run()
